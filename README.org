...
Changing user-interface from Hydra to Transient and the other way
around is now possible without restarting the Uniline session.

Take a look at the new "Customize" entry in the Uniline menu. This
menu is available:
...
Note that those changes are for the current session only. To
permanently choose Hydra or Transient, change your =~/.emacs=.

See [[#hydra-or-transient][Hydra or Transient?]]
