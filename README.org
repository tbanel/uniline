...
Sometimes while drawing a line, the line gets broken and
misaligned. The issue was discovered and diagnosed by a user. See
[[#language-environment][Language environment]] for details.
...
The workaround for the time being, is to change the language while
using Uniline. To do so, type:
...
A more satisfying solution is being investigated. Stay tuned.
