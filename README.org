...
*New*: Customization across sessions
...
The easiest way to manage this switch is through the Emacs standard customization infrastructure. Type:

=M-x customize-variable uniline-interface-type=

and permanently save your choice for future sessions.
...
After discussions with the community, it appears that =Transient= is the
way to go. Therefore the switch now have =:transient= as its default
value.
...
as it used to. Therfore, the =Hydra= package must be installed
...
choosen. (There is no way to make the dependencies conditional).
...
Actually, =Uniline= ships with =:transient= as the default
value. Therefore, there in nothing to do to benefit from the =Transient=
interface.
...
This can be either =:hydra= or =:transient=. See the
chapter "Hydra or Transient?". This setting has no immediate
effect. It will be taken into account only after upgrading the =Uniline=
package (or re-installing the same version).

It can also be taken into account after re-loading the =uniline.el=
file. Or after recompiling it and re-loading =uniline.elc=.
...
#+begin_src elisp
(load-file "uniline.el")   ;; interpreted form
(load-file "uniline.elc")  ;; byte-compiled form
#+end_src

You should prefer the byte-compiled form over the interpreted form,
because there are a lot of optimizations performed at compile time.
