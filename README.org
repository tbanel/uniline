...
transient, core). Loading =uniline-hydra.elc= or =uniline-transient.elc=
automatically loads =uniline-core.elc=.
...
Reducing =Transient= hints to single lines like under the =Hydra=
interface is currently not supported. It is on the todo-list. However,
this can be globally customized through this =Transient= customization:

#+begin_example
M-x customize-variable transient-show-popup
#+end_example
