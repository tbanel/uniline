...
*New*: =Transient= interface (as an alternative to =Hydra=). Still
experimental.
...
- [[#hydra-or-tansient][Hydra or Tansient?]]
...
There are no customizeable =Uniline= variables (not yet). This does not
...
=set-default=. Using =setq= instead would assign the value =1= only in the
=~/.emacs= buffer.
...
=Transient= interface was added recently to =Uniline=. It is still
experimental.

Beware that in its current state, =Uniline= as packaged into =Melpa=
declares =Hydra= as a dependency. There is no way to make the
dependencies conditional.

=Transient= is not declared as a dependency. Therefore, when choosing
=Transient=, the =Transient= package should be installed manually (or
automatically as a dependency on an unrelated package, like =Magit=). It
should be installed before installing =Uniline=.

The alternative to an installation-time switch would be to release two
packages: =Uniline-hydra= and =Uniline-transient=. This would be quite
disturbing.
